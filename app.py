...
def get_stock_data(ticker):
    ...
    try:
        ...
        st.write(f"Creating yf.Ticker object for {ticker}")
        stock = yf.Ticker(ticker, session=session)
        
        st.write(f"Getting historical data for {ticker}")
        ...
        st.write(f"Last 5 closing prices: {hist['Close'].tail().tolist()}")
    ...
    for i, stock in enumerate(STI_STOCKS):
        ...
        if current_price and close_prices and len(close_prices) >= 2:
            ...
            if current_price < ma_50:
                ...
                st.write(f"📈 ABOVE MA: {stock} is {below_ma_pct}% above 50-MA")
        else:
